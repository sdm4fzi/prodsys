...
def get_requets_handler(
    ...
    if(request.request_type == request_module.RequestType.PRODUCTION and request.process.data.product_disassembly_dict.__len__() >=1):
...
class DisassemblyProcessHandler:
    ...
    def handle_request(self, process_request: request_module.Request) -> Generator:
        ...
        with resource.request() as req:
            ...
            yield from self.disassembly(process_request)
    ...
    def disassembly(self, process_request: request_module.Request) -> Generator: 
        resource = process_request.get_resource()
        process = process_request.get_process()
        product = process_request.get_item()
        ...
        filtered_ports = [
        p for p in resource.ports
        if getattr(getattr(p, "data", p), "interface_type", None)
        in (PortInterfaceType.INPUT_OUTPUT, PortInterfaceType.OUTPUT)
