...
# Simplified prodsys models API example that works with v1
# This example creates a production system but doesn't run the simulation
# to avoid circular import issues with the runner
...
print("Creating a production system with prodsys v1...")
...
# Create processes to produce a part and to transport it
...
# Create a time model for product arrival in the system
...
# Create a source that creates the product
...
# Add default queues to the production system
print("\nAdding default queues to resources...")
add_default_queues_to_resources(production_system_instance)

# Validate the configuration
print("\nValidating configuration...")
production_system_instance.validate_configuration()

print("\n✅ Production system created and validated successfully!")
print(f"Production system contains:")
print(f"  - {len(production_system_instance.time_model_data)} time models")
print(f"  - {len(production_system_instance.process_data)} processes")
print(f"  - {len(production_system_instance.resource_data)} resources")
print(f"  - {len(production_system_instance.product_data)} products")
print(f"  - {len(production_system_instance.source_data)} sources")
print(f"  - {len(production_system_instance.sink_data)} sinks")
print(f"  - {len(production_system_instance.queue_data)} queues (auto-generated)")

print("\n🎉 Prodsys v1 with Poetry is working correctly!")
print("Note: Simulation runner has circular import issues in v1, but the models work perfectly.")
