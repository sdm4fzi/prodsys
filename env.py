...
class Environment(simpy.Environment):
    ...
    def initialize_simulation(self):
        ...
        self.time_model_factory = time_model.TimeModelFactory(self.loader.time_model_data)
        self.time_model_factory.create_time_models()

        self.state_factory = state.StateFactory(self.loader.state_data, self, self.time_model_factory)
        self.state_factory.create_states()

        self.process_factory = process.ProcessFactory(self.loader.process_data, self.time_model_factory)
        ...
        self.resource_factory = resources.ResourceFactory(self.loader.resource_data, self, self.process_factory, self.state_factory, self.queue_factory)
        self.resource_factory.create_resources()

        self.material_factory = material.MaterialFactory(self.loader.material_data, self, self.process_factory)

        self.sink_factory = sink.SinkFactory(self.loader.sink_data, self, self.material_factory, self.queue_factory)
        self.sink_factory.create_sinks()

        router_object = router.SimpleRouter(env=self, resource_process_registry=self.resource_factory, routing_heuristic=router.random_router, sink_registry=self.sink_factory)
        router_dict = {'SimpleRouter': router_object}

        self.source_factory = source.SourceFactory(self.loader.source_data, self, self.material_factory, self.time_model_factory, self.queue_factory, router_dict)
        ...
        for r in self.resource_factory.resources:
            ...
            for __state in all_states:
                self.data_collector.register_patch(__state.state_info, attr=['log_start_state', 'log_start_interrupt_state', 'log_end_interrupt_state', 'log_end_state'], 
