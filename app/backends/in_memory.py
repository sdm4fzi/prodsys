...
class InMemoryBackend:
    ...
    def create_project(self, project: Project) -> Project:
        if project.ID in self.database:
            raise HTTPException(
                409, f"Project {project.ID} already exists. Try updating it with put."
            )
    ...
    def get_adapters(self, project_id: str) -> List[ProductionSystemData]:
        project = self.get_project(project_id)
        return project.adapters

    def get_adapter(self, project_id: str, adapter_id: str) -> ProductionSystemData:
        project = self.get_project(project_id)
    ...
    def create_adapter(
        self, project_id: str, adapter: ProductionSystemData
    ) -> ProductionSystemData:
        project = self.get_project(project_id)
        if adapter.ID in [a.ID for a in project.adapters]:
            raise HTTPException(
                409,
                f"Adapter {adapter.ID} already exists in project {project_id}. Try updating it with put.",
            )
        project.adapters.append(adapter)
        return adapter

    def update_adapter(
        self, project_id: str, adapter_id: str, adapter: ProductionSystemData
    ) -> ProductionSystemData:
    ...
    def get_performance(self, project_id: str, adapter_id: str) -> str:
        ...
        if adapter_id not in project.performances:
            raise HTTPException(
                404,
                f"Performance for adapter {adapter_id} not found in project {project_id}",
            )
    ...
    def create_performance(
        ...
        if adapter_id in project.performances:
            raise HTTPException(
                409,
                f"Performance for adapter {adapter_id} already exists in project {project_id}. Try updating it with put.",
            )
    ...
    def update_performance(
        ...
        if adapter_id not in project.performances:
            raise HTTPException(
                404,
                f"Performance for adapter {adapter_id} not found in project {project_id}. Try creating it with post.",
            )
        project.performances[adapter_id] = performance
        return performance
    ...
    def get_post_processor(self, project_id: str, adapter_id: str) -> PostProcessor:
        ...
        if not project.post_processor:
            raise HTTPException(
                404,
                f"PostProcessor not found in adapter {adapter_id} of project {project_id}",
            )
        return project.post_processor

    def create_post_processor(
        self, project_id: str, adapter_id: str, post_processor: PostProcessor
    ) -> PostProcessor:
        project = self.get_project(project_id)
        adapter = self.get_adapter(project_id, adapter_id)
        if project.post_processor:
            raise HTTPException(
                409,
                f"PostProcessor already exists in project {project_id}. Try updating it with put.",
            )
    ...
    def update_post_processor(
        ...
        if not project.post_processor:
            raise HTTPException(
                404,
                f"PostProcessor not found in project {project_id}. Try creating it with post.",
            )
        project.post_processor = post_processor
        return post_processor
    ...
    def delete_post_processor(self, project_id: str, adapter_id: str):
        ...
        if not project.post_processor:
            raise HTTPException(
                404, f"PostProcessor not found in project {project_id}."
            )
    ...
    def save_optimizer_hyperparameters(
        ...
        with open(
            f"data/{project_id}/{adapter_id}/optimizer_hyperparameters.json", "w"
        ) as file:
    ...
    def get_last_optimizer_hyperparameters(
        ...
        with open(
            f"data/{project_id}/{adapter_id}/optimizer_hyperparameters.json", "r"
        ) as file:
