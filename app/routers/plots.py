...
async def get_throughput_time_distribution(
    ...
    try:
        ...
        html_content = kpi_visualization.plot_throughput_time_distribution(
            post_processor, return_html=True
        )
...
async def get_line_balance_kpis(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_line_balance_kpis(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
async def get_production_flow_rate_per_product(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_production_flow_rate_per_product(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
async def get_resource_utilization(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_boxplot_resource_utilization(
            post_processor, return_html=True
        )
...
async def get_throughput_time_over_time(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_throughput_time_over_time(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
async def get_time_per_state_of_resources(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_time_per_state_of_resources(
            post_processor, normalized=normalized, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_util_WIP_resource(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_util_WIP_resource(
            post_processor, normalized=normalized, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
async def get_transport_utilization_over_time(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_transport_utilization_over_time(
            post_processor,
            transport_resource_names=transport_resource_names,
            return_html=True,
        )
        return HTMLResponse(content=html_content, media_type="text/html")
    except Exception as e:
...
async def get_WIP_with_range(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_WIP_with_range(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
async def get_WIP_per_resource(
    ...
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_WIP_per_resource(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
...
@router.get("/primitive_WIP")
async def get_primitive_WIP(
    project_id: str,
    adapter_id: str,
):
    try:
        post_processor = prodsys_backend.get_post_processor(project_id, adapter_id)
        html_content = kpi_visualization.plot_primitive_WIP(
            post_processor, return_html=True
        )
        return HTMLResponse(content=html_content)
    except Exception as e:
