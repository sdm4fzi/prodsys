...
chamber1_input_queue = psx.Queue(
    ID="chamber1_input_queue",
    capacity=4,
    location=[5, 0],
    interface_type=port_data.PortInterfaceType.INPUT,
)

chamber1_output_queue = psx.Queue(
    ID="chamber1_output_queue",
    capacity=4,
    location=[5, 0],
    interface_type=port_data.PortInterfaceType.OUTPUT,
)

chamber2_output_queue = psx.Queue(
    ID="chamber2_output_queue",
    capacity=4,
    location=[5.5, 0],
    interface_type=port_data.PortInterfaceType.OUTPUT,
)

chamber2_input_queue = psx.Queue(
    ID="chamber2_input_queue",
    capacity=4,
    location=[5.5, 0],
    interface_type=port_data.PortInterfaceType.INPUT,
)
...
chamber1 = psx.Resource(
    [p1],
    [4.5, 0],
    2,
    ID="chamber1",
)
chamber1.ports = [chamber1_input_queue, chamber1_output_queue]

chamber2 = psx.Resource(
    [p2],
    [5.5, 0],
    4,
    ID="chamber2",
)
chamber2.ports = [chamber2_input_queue, chamber2_output_queue]

transport = psx.Resource([tp], [2, 0], 1, ID="transport")

machine_handler = psx.Resource([tp], [5, 0], 1, ID="machine_handler")


machine = psx.SystemResource(
    [process_model],
    [5, 0],
    6,
    ID="machine",
    subresource_ids=["chamber1", "chamber2", "machine_handler"],
)
...
system = psx.ProductionSystem(
    [machine, chamber1, chamber2, transport, machine_handler], [source1]  , [sink1]
)
