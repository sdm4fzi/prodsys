"""
Link-transport (AGV) simulation using a draw.io XML file for layout input.

.. note::
    This is the legacy XML-driven approach.  For the modern, fully model-driven
    equivalent see ``link_transport_with_model_layout_example.py``, which uses
    ``LayoutData`` / ``LayoutAreaData`` / ``ObstacleData`` directly in the
...
def main():
    ...
    node_link_generation.plot_layout(production_system_instance, title="AGV Layout – XML-defined areas")
