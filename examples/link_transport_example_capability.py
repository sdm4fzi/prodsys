...
required_capability_process_transport = psx.RequiredCapabilityProcess(
    ID="required_capability_process_transport",
    capability="AGV transport"
)
...
product01 = psx.Product(
    ...
    transport_process=required_capability_process_transport,
    ID="product01",
)
...
ltp01 = psx.LinkTransportProcess(time_model=time_model_agv, ID="ltp01", capability="AGV transport", links=links)
...
adapter.write_data("link_transport_example_cap.json")

from prodsys.adapters import JsonProductionSystemAdapter
import prodsys

adapter = JsonProductionSystemAdapter()
adapter.read_data("link_transport_example_cap.json")
