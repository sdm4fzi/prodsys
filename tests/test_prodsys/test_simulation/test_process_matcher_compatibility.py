...
from prodsys.simulation.process_matcher import (
    ProcessMatcher,
    ResourceCompatibilityKey,
...
class TestProcessMatcherCompatibility:
    ...
    @pytest.fixture
    def process_matcher_link_transport_with_capabilities(
        self, link_transport_with_capabilities_system
    ):
        """Create a ProcessMatcher instance for link transport with capabilities system."""
        from prodsys import runner

        runner_instance = runner.Runner(
            production_system_data=link_transport_with_capabilities_system
        )
        runner_instance.initialize_simulation()
        return runner_instance.product_factory.router.request_handler.process_matcher

    @pytest.fixture
    def process_matcher_link_transport_without_capabilities(
        self, link_transport_without_capabilities_system
    ):
        """Create a ProcessMatcher instance for link transport without capabilities system."""
        from prodsys import runner

        runner_instance = runner.Runner(
            production_system_data=link_transport_without_capabilities_system
        )
        runner_instance.initialize_simulation()
        return runner_instance.product_factory.router.request_handler.process_matcher
    ...
    def test_precompute_production_compatibility_most_trivial(
        self, process_matcher_most_trivial
    ):
        ...
        dummy_products = process_matcher_most_trivial._create_dummy_products()
        process_matcher_most_trivial._precompute_production_compatibility(
            dummy_products
        )
        ...
        production_keys = list(
            process_matcher_most_trivial.production_compatibility.keys()
        )
        assert any("ProductionProcess:p1" in str(key) for key in production_keys)

    def test_precompute_production_compatibility_primitive_complex(
        self, process_matcher_primitive_complex
    ):
        ...
        process_matcher_primitive_complex._precompute_production_compatibility(
            dummy_products
        )
        ...
        # Check for specific process signatures
        production_keys = list(
            process_matcher_primitive_complex.production_compatibility.keys()
        )
        assert any("ProductionProcess:p1" in str(key) for key in production_keys)
        assert any("ProductionProcess:p2" in str(key) for key in production_keys)

    def test_precompute_transport_compatibility_most_trivial(
        self, process_matcher_most_trivial
    ):
    ...
    def test_precompute_transport_compatibility_link_transport_with_capabilities(
        self, process_matcher_link_transport_with_capabilities
    ):
        ...
        dummy_products = (
            process_matcher_link_transport_with_capabilities._create_dummy_products()
        )
        process_matcher_link_transport_with_capabilities._precompute_transport_compatibility(
            dummy_products
        )
        ...
        assert (
            len(process_matcher_link_transport_with_capabilities.reachability_cache) > 0
        )
        ...
        all_locations = (
            process_matcher_link_transport_with_capabilities._get_all_locations()
        )
        ...
        expected_node_connections = [
            ("source01_default_output_queue", "source01_default_output_queue"),
            ("source01_default_output_queue", "resource01_default_queue"),
            ("source01_default_output_queue", "node3"),
            ("source01_default_output_queue", "node1"),
            ("source01_default_output_queue", "node2"),
            ("resource01_default_queue", "source01_default_output_queue"),
            ("resource01_default_queue", "resource01_default_queue"),
            ("resource01_default_queue", "node3"),
            ("resource01_default_queue", "node1"),
            ("resource01_default_queue", "node2"),
            ("node3", "source01_default_output_queue"),
            ("node3", "resource01_default_queue"),
            ("node3", "node3"),
            ("node3", "node1"),
            ("node3", "node2"),
            ("node1", "source01_default_output_queue"),
            ("node1", "resource01_default_queue"),
            ("node1", "node3"),
            ("node1", "node1"),
            ("node1", "node2"),
            ("node2", "source01_default_output_queue"),
            ("node2", "resource01_default_queue"),
            ("node2", "node3"),
            ("node2", "node1"),
            ("node2", "node2"),
        ]
        for origin_id, target_id in expected_node_connections:
            ...
            assert (
                cache_key
                in process_matcher_link_transport_with_capabilities.reachability_cache
    ...
    def test_precompute_transport_compatibility_link_transport_without_capabilities(
        self, process_matcher_link_transport_without_capabilities
    ):
        ...
        dummy_products = (
            process_matcher_link_transport_without_capabilities._create_dummy_products()
        )
        process_matcher_link_transport_without_capabilities._precompute_transport_compatibility(
            dummy_products
        )
        ...
        assert (
            len(process_matcher_link_transport_without_capabilities.reachability_cache)
            > 0
        )
    ...
    def test_add_to_transport_compatibility(self, process_matcher_most_trivial):
        ...
        process_matcher_most_trivial._add_to_transport_compatibility(
            key, mock_resource, mock_process
        )
        ...
        assert (
            mock_resource,
            mock_process,
        ) in process_matcher_most_trivial.transport_compatibility[key]

    def test_add_to_transport_compatibility_duplicate_prevention(
        self, process_matcher_most_trivial
    ):
    ...
    def test_handle_required_capability_process(
        self, process_matcher_link_transport_with_capabilities
    ):
        ...
        mock_requested_process.get_process_signature.return_value = (
            "RequiredCapabilityProcess:rtp01"
        )
        ...
        process_matcher_link_transport_with_capabilities._handle_required_capability_process(
            mock_request,
            mock_requested_process,
            mock_resource,
            mock_offered_process,
            mock_origin,
            mock_target,
        )
    ...
    def test_handle_required_capability_process_capability_mismatch(
        self, process_matcher_link_transport_with_capabilities
    ):
        ...
        assert (
            "source01",
            "sink01",
        ) not in process_matcher_link_transport_with_capabilities.reachability_cache

    @patch("prodsys.simulation.route_finder.find_route")
    def test_handle_link_transport_process(
        self, mock_find_route, process_matcher_link_transport_without_capabilities
    ):
        ...
        assert (
            "source01",
            "sink01",
        ) in process_matcher_link_transport_without_capabilities.reachability_cache
        assert len(process_matcher_link_transport_without_capabilities.route_cache) > 0

    @patch("prodsys.simulation.route_finder.find_route")
    def test_handle_link_transport_process_no_route(
        self, mock_find_route, process_matcher_link_transport_without_capabilities
    ):
        ...
        assert (
            "source01",
            "sink01",
        ) not in process_matcher_link_transport_without_capabilities.reachability_cache
    ...
    def test_precompute_compatibility_tables_integration(
        self, process_matcher_most_trivial
    ):
    ...
    def test_get_compatible_production_processes(self, process_matcher_most_trivial):
        ...
        required_processes = process_matcher_most_trivial.get_all_required_processes(
            product
        )

        # Get compatible resources
        compatible_resources = process_matcher_most_trivial.get_compatible(
            required_processes
        )
        ...
        for resource, process_obj in compatible_resources:
            ...
            assert isinstance(
                ...
                (
                    ...
                    process.ReworkProcess,
                    process.CompoundProcess,
                ),
            )
    ...
    def test_get_rework_compatible_no_rework_processes(
        self, process_matcher_most_trivial
    ):
    ...
    def test_empty_system_compatibility(self):
        ...
        runner_instance.initialize_simulation()
        process_matcher = (
            runner_instance.product_factory.router.request_handler.process_matcher
        )
