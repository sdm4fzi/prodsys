...
def simulation_adapter() -> ProductionSystemData:
    ...
    sink3 = psx.Sink(mutterproduct, [20,10], "muttersink")
...
def test_hashing(simulation_adapter: ProductionSystemData):
    ...
    print(hash_str)
    assert hash_str == "7a4a191c7117c128cb01e9cba8005697"
...
def test_run_simulation(simulation_adapter: ProductionSystemData):
    ...
    for kpi in post_processor.machine_state_KPIS:
        if kpi.name == "productive_time" and kpi.resource == "assemblymachine_1":
            assert kpi.value > 70 and kpi.value < 86
    ...
    tank_output = 0.0 
    pump_output = 0.0 
    valve_block_output = 0.0 
    for kpi in post_processor.throughput_and_output_KPIs:
        if kpi.name == "output":
            if kpi.product_type == "tank":
                tank_output = kpi.value
            if kpi.product_type == "valve block":
                valve_block_output = kpi.value
            if kpi.product_type == "pump":
                pump_output = kpi.value
    assert tank_output <= valve_block_output
    assert tank_output <= pump_output
