...
def simulation_adapter() -> JsonProductionSystemAdapter:
    ...
    reworker = psx.ProductionResource([rework_process], [8, 0], 1, ID="reworker")
    ...
    system = psx.ProductionSystem([machine, reworker, transport], [source1], [sink1])
...
def test_hashing(simulation_adapter: JsonProductionSystemAdapter):
    ...
    assert hash_str == "ff2fe6c7bf4e5c88e5039d406044b2c6"
...
def test_run_simulation(simulation_adapter: JsonProductionSystemAdapter):
    ...
    assert runner_instance.env.now == 200
